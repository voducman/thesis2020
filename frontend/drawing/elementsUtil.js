...
const elementsUtil = (function(){
    ...
    function initArrayValuesFrom(min, max) {
        let values = [],
            range = max - min
        for (let i = 0, iLen = range / 0.1; i < iLen; i++) {
            values.push(parseFloat(min) + i*0.1)
        }
    ...
    return {
        ...
        renderInput: function(id, width, height){
            $(`#${id}`).append(`
                <div>
                    ...
                    style="box-sizing: border-box; width:${width}px; height:${height}px; border-width: 0px; font-size: 10px;"/>
                </div>`); 
        },
        ...
        renderProgressBar: function(id, width, height){
            $(`#${id}`).append(`
            <div class="progress" id="child-${id}" style="height:${height}px; width: ${width}px; border-radius: 0; background-color: white;">
                <div class="progress-bar role="progressbar" aria-valuenow="25"
                            aria-valuemin="0" aria-valuemax="100" style="width:50%; background-color: green;">
                     <span style="line-height: ${height}px; display: block;">25%</span>
                </div>
            </div>`);  
        },
