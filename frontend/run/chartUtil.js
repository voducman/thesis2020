  ...
  return {
    ...
    renderPieChart: function (id) {
      ...
      chartProperties.pushDatas = function(tag1, tag2, tag3, tag4, tag5){
        ...
        if (!isNaN(tag1)) chartist.data.series.push(tag1);
        if (!isNaN(tag2)) chartist.data.series.push(tag2);
        if (!isNaN(tag3)) chartist.data.series.push(tag3);
        if (!isNaN(tag4)) chartist.data.series.push(tag4);
        if (!isNaN(tag5)) chartist.data.series.push(tag5);
    ...
    renderRadialGaugeChart: function (id) {
      ...
      gaugeProperties.updateOptions = function(scale, label, bgColor, fgColor, min, max){
        ...
        options.min = min || 0;
        options.max = max || 100;
        options.gaugeColor = bgColor;
        options.levelColors = [fgColor];
