...
initDataFromDB()
.then((data) => {
    numRow = Table.renderTagTable(data);
    initPagigation(numRow);
})
...
$('#last').click(function(){
    for (let i = 1; i<= numRow; i++){
        if (i > 10*Math.floor(numRow/10)) $(`#tag-${i}`).show();
        else         $(`#tag-${i}`).hide();
    }
...
function initPagigation(numrow){
    ...
    if (numrow < 10){
        ...
        $('#last').parent().hide();
        if (numrow == 0) {
            $('#pagination p').text(`Showing 0 to ${numrow} of ${numrow} entries`);
        }else{
